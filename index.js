...
let locationFile = process.argv[2];//location file
...
// function to permutate locations point
function perm(xs) {
...
(async function ReadCred() {
    try {
        ...
        //read loc file 
        let locationFileStr = await fs.promises.readFile(locationFile, "utf-8");
        let locationJSON = JSON.parse(locationFileStr);
        let startPt = locationJSON.startPt;
        let endPt = locationJSON.endPt;
        let URL = locationJSON.URL;
        let locs = locationJSON.stopages;

        //permutations
        let locationPermute = perm(locs);

        console.log(" Total Possible Routes -  "+ locationPermute.length);
        ...
        do{
            ...
            if(locationPermute.length > 0) 
                Route = Route.concat(locationPermute[i]);
        ...
        }while(i < locationPermute.length)
        ...
        await fs.promises.writeFile('OptimalPath.html', Mhtml);
        ...
        let page3 = pages3[0];
        await page3.goto('file://'+ path +'/OptimalPath.html',{ waitUntil: 'networkidle0' });
        let pdf = await generatePDF(page3);
